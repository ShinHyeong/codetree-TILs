...
def shot(k):
    ...
    # k가 1 증가할 때 좌표의 변화 : y+=1 => 가변값 - 고정값 = k - 구간시작값
        return 2, (0, 0 +(k -0*n-1))

    elif k<=n*2: #[1*n+1,2*n] 서
    #(n,k,x)->(3,4,0),(3,5,1),(3,6,2) 
    # k가 1 증가할 때 좌표의 변화 : x+=1 => 가변값 - 고정값 = k - 구간시작값
        return 3, (0 +(k -1*n-1), n-1)

    elif k<=n*3: #[2*n+1,3*n] 북
    #(n,k,y)->(3,7,2),(3,8,1),(3,9,0) 
    # k가 1 증가할 때 좌표의 변화 : y-=1 => 고정값 - 가변값 = 구간시작값 -k
        return 0, (n-1, n-1 -(2*n+-k))

    else: #k<=n*4 #[3*n+1,4*n] 동
    #(n,k,x)->(3,10,2),(3,11,1),(3,12,0) 
    # k가 1 증가할 때 좌표의 변화 : x-=1 => 고정값 - 가변값 = 구간시작값 -k
        return 1, (n-1 -(3*n+1-k) ,0)
